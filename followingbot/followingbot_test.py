...
try:
    ...
    while True:
        ...
        for detection in detections:
	        #print(detection)
            Data = "classID: {} conf:{}".format(detection.ClassID, detection.Confidence)
            print(Data)
            if detection.ClassID == 3 and detection.Confidence > 0.6:
                goval = True
        ...
        if goval == True:
            ...
            Motor.MotorRun(0, 'forward', left_value)
            Motor.MotorRun(1, 'forward', right_value)
        else:
            Motor.AllMotorStop()
